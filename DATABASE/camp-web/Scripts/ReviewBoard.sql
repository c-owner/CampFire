...
SELECT BNO, TITLE, CONTENT, WRITER, REGDATE, UPDATEDATE, LIKECNT, READCNT, REPLYCNT, STAR, 
THUMB, CAMPNAME FROM ( SELECT /*+ INDEX_DESC(TBL_REVIEW_BOARD PK_REVIEW_BOARD) */ ROWNUM rn, 
BNO, TITLE, CONTENT, WRITER, REGDATE, UPDATEDATE, LIKECNT, READCNT, REPLYCNT, STAR, THUMB, 
CAMPNAME FROM TBL_REVIEW_BOARD WHERE TITLE LIKE '%'||'자 난 키워드를 던졌고 넌 키워드를 문것이여'||'%' OR CONTENT 
LIKE '%'||'자 난 키워드를 던졌고 넌 키워드를 문것이여'||'%' OR WRITER LIKE '%'||'자 난 키워드를 던졌고 넌 키워드를 문것이여'||'%' 
OR ADDRESS LIKE '%'||'자 난 키워드를 던졌고 넌 키워드를 문것이여'||'%' OR ADDRESS_DETAIL LIKE '%'||'자 난 키워드를 
던졌고 넌 키워드를 문것이여'||'%' OR ADDRESS_ETC LIKE '%'||'자 난 키워드를 던졌고 넌 키워드를 문것이여'||'%' OR CAMPNAME 
LIKE '%'||'자 난 키워드를 던졌고 넌 키워드를 문것이여'||'%') WHERE ROWNUM <= 1 * 12 ) AND rn > ((1 - 1) * 12));
---------------------------
-- 댓글 테이블 추가
