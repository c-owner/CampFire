...
SELECT BNO, TITLE, CONTENT, WRITER, REGDATE, UPDATEDATE, LIKECNT, READCNT, REPLYCNT, STAR, THUMB, CAMPNAME, CATEGORIES 
FROM (SELECT /*+ INDEX_DESC(TBL_REVIEW_BOARD PK_REVIEW_BOARD) */ ROWNUM rn, BNO, TITLE, CONTENT, WRITER, REGDATE, 
UPDATEDATE, LIKECNT, READCNT, REPLYCNT, STAR, THUMB, CAMPNAME, CATEGORIES 
FROM TBL_REVIEW_BOARD WHERE ROWNUM <= 1 * 12 AND (CATEGORIES LIKE 'A7') 
AND REGDATE BETWEEN SYSDATE-1 AND SYSDATE 
ORDER BY REGDATE DESC)
WHERE rn > (1 - 1) * 12
