...
import com.campfire.service.TipBoardReplyService;
...
@RestController
@AllArgsConstructor
@RequestMapping("/tipReplies/")
public class TipBoardReplyController {
	private TipBoardReplyService service;
	
	//댓글 등록
	@PostMapping(value="/tipReplyWrite", consumes="application/json", produces="text/html; charset=utf-8")
	public ResponseEntity<String> register(@RequestBody TipBoardReplyVO g_vo){
	...
	//해당 게시글의 댓글 목록
	@GetMapping(value="/tipReplyList/{bno}/{page}", produces= {MediaType.APPLICATION_JSON_UTF8_VALUE, MediaType.APPLICATION_XML_VALUE})
	...
	public ResponseEntity<TipBoardReplyVO> get(@PathVariable("rno") Long rno){
		return new ResponseEntity<TipBoardReplyVO>(service.get(rno), HttpStatus.OK);
	}
	
	//댓글 수정
	@PatchMapping(value="/{rno}", consumes="application/json", produces="text/html; charset=utf-8")
	public ResponseEntity<String> modify(@PathVariable("rno") Long rno, @RequestBody TipBoardReplyVO g_vo){
		g_vo.setRno(rno);
